...
def val(models, vals_loader, loss_func, n_labels):
    models.eval()
    val_loss = metrics.LossAverage()
    val_dice = metrics.DiceAverage(n_labels)
    with torch.no_grad():
        for idx, (data, target) in tqdm(enumerate(vals_loader), total=len(vals_loader)):
            ...
            val_loss.update(losses.item(), data.size(0))
            val_dice.update(output, target)
    vals_log = OrderedDict({'Val_Loss': val_loss.avg, 'Val_dice_liver': val_dice.avg[1]})
    if n_labels == 3:
        vals_log.update({'Val_dice_tumor': val_dice.avg[2]})
    return vals_log


def train(models, train_loaders, optimizers, loss_func, n_labels, alphas):
    print("=======Epoch:{}=======lr:{}".format(epoch, optimizers.state_dict()['param_groups'][0]['lr']))
    models.train()
    train_loss = metrics.LossAverage()
    train_dice = metrics.DiceAverage(n_labels)

    for idx, (data, target) in tqdm(enumerate(train_loaders), total=len(train_loaders)):
        data, target = data.float(), target.long()
        target = common.to_one_hot_3d(target, n_labels)
        data, target = data.to(device), target.to(device)
        ...
        losses = loss3 + alphas * (loss0 + loss1 + loss2)
        losses.backward()
        optimizers.step()
    ...
    val_logs = OrderedDict({'Train_Loss': train_loss.avg, 'Train_dice_liver': train_dice.avg[1]})
    if n_labels == 3:
        val_logs.update({'Train_dice_tumor': train_dice.avg[2]})
    return val_logs
