...
def predict_one_img(models, img_datasets, arg):
    dataloader = DataLoader(dataset=img_datasets, batch_size=1, num_workers=0, shuffle=False)
    models.eval()
    test_dices = DiceAverage(arg.n_labels)
    target = to_one_hot_3d(img_datasets.label, arg.n_labels)
    
    with torch.no_grad():
        for data in tqdm(dataloader, total=len(dataloader)):
            data = data.to(device)
            output = models(data)
            img_datasets.update_result(output.detach().cpu())

    pred = img_datasets.recompone_result()
    pred = torch.argmax(pred, dim=1)

    pred_imgs = common.to_one_hot_3d(pred, arg.n_labels)
    test_dices.update(pred_imgs, target)
    
    test_dices = OrderedDict({'Dice_liver': test_dices.avg[1]})
    if arg.n_labels == 3:
        test_dices.update({'Dice_tumor': test_dices.avg[2]})
