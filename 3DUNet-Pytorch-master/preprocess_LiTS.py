...
class LITS_preprocess:
    def __init__(self, raw_dataset_paths, fixed_dataset_paths, arg):
        self.raw_root_path = raw_dataset_paths
        self.fixed_path = fixed_dataset_paths
        self.classes = arg.n_labels  # 分割类别数（只分割肝脏为2，或者分割肝脏和肿瘤为3）
        self.upper = arg.upper
        self.lower = arg.lower
        self.expand_slice = arg.expand_slice  # 轴向外侧扩张的slice数量
        self.size = arg.min_slices  # 取样的slice数量
        self.xy_down_scale = arg.xy_down_scale
        self.slice_down_scale = arg.slice_down_scale

        self.valid_rate = arg.valid_rate
        ...
        for ct_file, i in zip(file_list, range(numbers)):
            ...
            if new_ct is not None and new_seg is not None:
                ...
                sitk.WriteImage(new_seg,
                                os.path.join(self.fixed_path, 'label',
    ...
    def process(self, ct_path, seg_path, classes=None):
        ...
        ct_array = ndimage.zoom(ct_array, (ct.GetSpacing()[-1] / self.slice_down_scale,
                                           self.xy_down_scale, self.xy_down_scale), order=3)
        seg_array = ndimage.zoom(seg_array, (ct.GetSpacing()[-1] / self.slice_down_scale,
...
if __name__ == '__main__':
    raw_dataset_path = '/mnt/sdc1/EZY/LITS/train'
    fixed_dataset_path = '/mnt/sdc1/EZY/LITS——FIXED'
